...
{
    const LocalizationBins bins =
        LocalizationBins::CreateFromValues(std::vector<double>{1.99, 1.01, 2.0, 1.0}, true, 0.1);
    ASSERT_EQ(bins.getNumBins(), 2);
    ASSERT_EQ(bins.getBinLower(0), 1.0);
    ASSERT_EQ(bins.getBinUpper(1), 2.0);
...
{
    const LocalizationBins bins = LocalizationBins::CreateFromValues(std::vector<double>{2.0, 1.0}, false);
    ASSERT_MYEXCEPTION_MESSAGE(LocalizationBins::CreateFromValues(std::vector<double>{1.0}, false),
                               "LocalizationBins::CreateFromValues need at least 2 values");
    ASSERT_MYEXCEPTION_MESSAGE(LocalizationBins::CreateFromValues(std::vector<double>{1.0}, false, -1.0),
                               "LocalizationBins::CreateFromValues minBinSize must be positive");
    ASSERT_MYEXCEPTION_MESSAGE(LocalizationBins::CreateFromValues(std::vector<double>{1.0, 1.1}, false, 1.0),
                               "LocalizationBins::CreateFromValues value too close");
}
