...
TEST(Polygon2AsRegion, Base)
{
    ...
    str::copy_if(allManifolds, std::back_inserter(pointManifolds),
                 [](const auto *m) { return m->getTopologyDimension() == Topology::Corner; });
    ...
    ASSERT_EQ(dynamic_cast<const DirectedEdge2AsManifold1<double> *>(point0_connections.at(0))->getName(),
              "foo_edge_0");
    ASSERT_EQ(dynamic_cast<const DirectedEdge2AsManifold1<double> *>(point0_connections.at(1))->getName(),
              "foo_edge_2");

    const auto point1_connections = triangleRegion.getConnectedHighers(*pointManifolds.at(1));
    ASSERT_EQ(point1_connections.size(), 2);
    ASSERT_EQ(dynamic_cast<const DirectedEdge2AsManifold1<double> *>(point1_connections.at(0))->getName(),
              "foo_edge_1");
    ASSERT_EQ(dynamic_cast<const DirectedEdge2AsManifold1<double> *>(point1_connections.at(1))->getName(),
              "foo_edge_0");
}
