...
TEST(IntervalTest, InterpolateFail)
{
    ...
    ASSERT_MYEXCEPTION_MESSAGE(interval.inverseInterpolate(1.0),
                               "Interval<double>::inverseInterpolate degenerate interval");
    interval = Interval<double>(0, std::numeric_limits<double>::epsilon());
    ASSERT_MYEXCEPTION_MESSAGE(interval.inverseInterpolate(std::numeric_limits<double>::max()),
                               "Interval<double>::inverseInterpolate degenerate interval");
}
