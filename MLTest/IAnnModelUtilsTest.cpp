...
NumericalCheck::NumericalCheck(const ML::IAnnModel &model, const ML::IAnnDataSet &dataSet, size_t layer, size_t param,
                               const ML::IParameterSet &parameterSet)
...
double NumericalCheck::Derivative(double x) const
{
    ...
    ML::IAnnModelUtils::setParameterDerivatives(m_model, *forwardResult, m_dataSet.getNthOutput(0), parameterSet,
                                                parameterSetDerivs);
    return parameterSetDerivs.at(m_layer)[m_param];
}

void DoCheckDerivatives(const ML::IAnnModel &model, const ML::IParameterSet &parameterSet,
                        std::span<const double> input, std::span<const double> output)
{
    ...
    for (size_t layer = 0; layer < parameterSet.getNumLayers(); ++layer)
    {
        ...
        for (size_t param = 0; param < parameterLayer.size(); ++param)
        {
            ...
            RealFunctionCheckDerivative::Check(nc, parameterLayer[param], 0.1);
        }
    }
}
} // namespace
...
TEST(IAnnModelUtilsTest, FeedForwardBasic)
{
    ...
    DoCheckDerivatives(model, parameterSet, std::vector<double>{0.3}, std::vector<double>{0.5});
}
...
TEST(IAnnModelUtilsTest, FeedForwardSlightlyMoreComplex)
{
    ...
    DoCheckDerivatives(model, parameterSet, std::vector<double>{0.3, 0.4}, std::vector<double>{0.5});
}
...
TEST(IAnnModelUtilsTest, SetParameterDerivativesSimple)
{
    ...
    DoCheckDerivatives(model, parameterSet, std::vector<double>{0.3, 0.4}, std::vector<double>{0.2});
}
...
TEST(IAnnModelUtilsTest, GeeksExample)
{
    ...
    DoCheckDerivatives(model, parameterSet, std::vector<double>{0.35, 0.7}, ideal);
}
...
TEST(IAnnModelUtilsTest, NeuroticExample)
{
    ...
    ML::ParameterSet parameterDerivs = ML::ParameterSet::CreateUsingDimensions(parameterSet);
    ML::IAnnModelUtils::setParameterDerivatives(model, *forwardResult, dataSet.getNthOutput(0), parameterSet,
                                                parameterDerivs);
    ...
    DoCheckDerivatives(model, parameterSet, dataSet.getNthInput(0), dataSet.getNthOutput(0));
}
...
TEST(IAnnModelUtilsTest, NumericalCheckSingleParameter)
{
    ...
    RealFunctionCheckDerivative::Check(nc2, singleParameter, 0.01);
}
...
TEST(IAnnModelUtilsTest, NumericalCheckNoBias2)
{
    ...
    DoCheckDerivatives(model, parameterSet, std::vector<double>{0.9, 0.4}, std::vector<double>{0.8});
}
...
TEST(IAnnModelUtilsTest, NumericalCheckSingleBias)
{
    ...
    DoCheckDerivatives(model, parameterSet, std::vector<double>{0.2, 0.4}, std::vector<double>{1.8});
}
