...
template <size_t N> struct ComparePointer
{
...
template <size_t N> Index<N>::Index(Key keys, Index1Factory &factory) : m_factory1(factory), m_keys(std::move(keys))
{
...
template <size_t N> const Interval<Rational> &Index<N>::getInterval(int n) const
{
    return m_factory1(m_keys.at(n))->getInterval();
}

template <size_t N> Level Index<N>::getLevel() const
{
    Level result = 0;
    for (auto key : m_keys)
    {
...
template <size_t N> Index<N>::Key Index<N>::getKey() const
{
    return m_keys;
}

template <size_t N> Rational Index<N>::getMeasure() const
{
    return std::accumulate(m_keys.begin(), m_keys.end(), Rational(1, 1), [this](const Rational &acc, Index1::Key key1) {
        return acc * m_factory1(key1)->getMeasure();
    });
}

template <size_t N> std::array<typename Index<N>::Key, NumKids<N>> Index<N>::refine() const
{
...
template <size_t N> Index<N>::Key Index<N>::GetParent(const Index<N>::Key &key)
{
...
template <size_t N> std::optional<typename Index<N>::Key> Index<N>::getAdjacentInDir(AdjacentDirection direction) const
{
...
template <size_t N> std::array<std::array<Rational, N>, NumKids<N>> Index<N>::getVerticesInVtkOrder() const
{
    throw MyException("Index<N>::getVerticesInVtkOrder() not implemented");
}

template <> std::array<std::array<Rational, 1>, 2> Index<1>::getVerticesInVtkOrder() const;

template <> std::array<std::array<Rational, 2>, 4> Index<2>::getVerticesInVtkOrder() const;

template <> std::array<std::array<Rational, 3>, 8> Index<3>::getVerticesInVtkOrder() const;
