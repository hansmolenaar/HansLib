...
template <size_t N> class IndexTree
{
  public:
    IndexTree();
    const Index<N> &getRoot() const;

    template <typename P> int refineLeaves(P &predicate);

    template <typename P> void refineUntilReady(P &predicate);

    template <typename A> void foreachLeaf(A &action) const;

    template <typename A> void foreachNode(A &action) const;
...
template <size_t N> IndexTree<N>::IndexTree() : m_root(m_factory.getRoot())
{
...
template <size_t N> std::vector<const Index<N> *> IndexTree<N>::getLeavesInFixedOrder() const
{
    std::vector<const Index<N> *> result(m_leaves.begin(), m_leaves.end());
    std::sort(result.begin(), result.end(), IntervalTree::ComparePointer<N>());
    return result;
...
template <size_t N> const Index<N> &IndexTree<N>::getRoot() const
{
    return *m_root;
}

template <size_t N> size_t IndexTree<N>::size() const
{
    return m_tree.size() + m_leaves.size();
}

template <size_t N> template <typename A> void IndexTree<N>::foreachLeaf(A &action) const
{
...
template <size_t N> template <typename A> void IndexTree<N>::foreachNode(A &action) const
{
...
template <size_t N> template <typename P> int IndexTree<N>::refineLeaves(P &predicate)
{
...
template <size_t N> template <typename P> void IndexTree<N>::refineUntilReady(P &predicate)
{
...
template <size_t N> std::optional<const Index<N> *> IndexTree<N>::get(const Index<N>::Key &key) const
{
    const auto ptr = m_factory.get(key);
    if (!ptr)
        return {};

    if (m_leaves.contains(*ptr))
        return ptr;
    if (m_tree.contains(*ptr))
        return ptr;
    return {};
}

template <size_t N> bool IndexTree<N>::contains(const Index<N>::Key &key) const
{
    return get(key).has_value();
}

template <size_t N> bool IndexTree<N>::isLeaf(const Index<N> &index) const
{
...
template <size_t N> const Index<N> &IndexTree<N>::getExistingSelfOrAncestor(Index<N>::Key key) const
{
    while (true)
    {
