...
template <typename T, size_t N> class KdTreeVertex;

template <typename T, size_t N> class KdTreeLeaf;
...
template <typename T, size_t N> class KdTree
{
  ...
  private:
    ...
    static KdTreeVertex<T, N> *BuildTree(std::array<std::span<KdTreePosition>, N> &orders, int depth,
                                         const std::span<const Point<T, N>> &points, std::vector<bool> &goRight,
                                         std::span<KdTreePosition> work, std::deque<KdTreeVertex<T, N>> &m_vertices);
...
template <typename T, size_t N> class KdTreeVertex
{
  public:
    ...
    void Traverse(const KdTree<T, N> &, IKdTreeTraversor<T, N> &, int level, Point<T, N> &lbound,
                  Point<T, N> &ubound) const;
    bool isLeaf() const
    {
  ...
  private:
    void TraverseRecur(const KdTree<T, N> &, IKdTreeTraversor<T, N> &, int level, std::array<T, N> &lbound,
                       std::array<T, N> &ubound) const;
...
template <typename T, size_t N> KdTreePosition KdTreeVertex<T, N>::getPosition() const
{
...
template <typename T, size_t N> class PreSorting
{
  public:
...
template <typename T, size_t N> KdTreeVertex<T, N>::KdTreeVertex(KdTreePosition pos) : m_position(pos)
{
}
...
template <typename T, size_t N>
void KdTreeVertex<T, N>::Traverse(const KdTree<T, N> &tree, IKdTreeTraversor<T, N> &traversor, int level,
                                  Point<T, N> &lbound, Point<T, N> &ubound) const
{
    TraverseRecur(tree, traversor, level, lbound, ubound);
}

template <typename T, size_t N>
void KdTreeVertex<T, N>::TraverseRecur(const KdTree<T, N> &tree, IKdTreeTraversor<T, N> &traversor, int level,
                                       std::array<T, N> &lbound, std::array<T, N> &ubound) const
{
    if (isLeaf())
    {
        HandleLeaf(tree, traversor, *this);
    ...
    else
    {
        ...
        if (m_left != nullptr)
        {
            ...
            {
                m_left->TraverseRecur(tree, traversor, level + 1, lbound, ubound);
            }
            ubound[dir] = storeVal;
        }
        if (m_right != nullptr)
        {
            ...
            if (traversor.DeterminOverlap(lbound, ubound) != KdTreeOverlap::NoOverlap)
            {
                m_right->TraverseRecur(tree, traversor, level + 1, lbound, ubound);
            }
            lbound[dir] = storeVal;
        }
    }
...
template <typename T, size_t N>
PreSorting<T, N>::PreSorting(int skipDim, std::span<const Point<T, N>> points) : m_currentDim(skipDim), m_points(points)
{
}

template <typename T, size_t N> bool PreSorting<T, N>::less(KdTreePosition n0, KdTreePosition n1) const
{
...
std::vector<KdTreePosition> KdTree<T, N>::SortPerDimension(int thisDim, const std::span<const Point<T, N>> points)
{
    ...
    std::sort(result.begin(), result.end(),
              [&preSorting](KdTreePosition n0, KdTreePosition n1) { return preSorting->less(n0, n1); });
    return result;
}

template <typename T, size_t N> KdTree<T, N>::KdTree(std::span<const Point<T, N>> points) : m_points(points)
{
    if (!m_points.empty())
    {
...
template <typename T, size_t N>
KdTreeVertex<T, N> *KdTree<T, N>::BuildTree(std::array<std::span<KdTreePosition>, N> &orders, int depth,
                                            const std::span<const Point<T, N>> &points, std::vector<bool> &goRight,
                                            std::span<KdTreePosition> work, std::deque<KdTreeVertex<T, N>> &buffer)
{
    ...
    else
    {
        ...
        for (int d = 0; d < N; ++d)
        {
            ...
            right[d] =
                std::span<KdTreePosition>(currentOrder.begin() + numLeft, currentOrder.begin() + numLeft + numRight);
        }
...
template <typename T, size_t N> std::vector<KdTreePosition> KdTree<T, N>::GetAllLeavesInOrder() const
{
    if (m_root != nullptr)
    {
...
template <typename T, size_t N> void KdTree<T, N>::Traverse(IKdTreeTraversor<T, N> &traversor)
{
    if (m_root != nullptr)
    {
...
template <typename T, size_t N> Point<T, N> KdTree<T, N>::GetPoint(KdTreePosition pos) const
{
    return m_points[pos];
}
