...
namespace
{
template <size_t N> using IndexSet = std::unordered_set<typename Index<N>::Key, typename Index<N>::KeyHasher>;

template <size_t N> void markUnbalancedLeaves(const IndexTree<N> &tree, const Index<N> &indx, IndexSet<N> &toRefine)
{
...
template <size_t N> IndexSet<N> RefineToBalance(const IndexTree<N> &tree)
{
    IndexSet<N> result;
...
template <size_t N> void DoBalance(IndexTree<N> &tree)
{
...
template <> void IntervalTree::Balance(IndexTree<1> &tree)
{
    DoBalance(tree);
}

template <> void IntervalTree::Balance(IndexTree<2> &tree)
{
    DoBalance(tree);
}

template <> void IntervalTree::Balance(IndexTree<3> &tree)
{
    DoBalance(tree);
}
