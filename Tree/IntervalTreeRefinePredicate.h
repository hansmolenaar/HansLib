...
template <size_t N> struct RefineToMaxLevel
{
    bool operator()(const Index<N> &indx) const
    {
        return indx.getLevel() < MaxLevel;
    };
    int MaxLevel;
};

template <size_t N> struct RefineIfContainsPoint
{
    bool operator()(const Index<N> &indx) const
    {
...
template <size_t N> struct RefineIfOverlaps
{
    bool operator()(const Index<N> &indx) const;
