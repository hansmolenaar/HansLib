...
template <typename T, size_t N> class KdTreeTraversorPointsInRange : public IKdTreeTraversor<T, N>
{
  public:
    ...
    const std::vector<KdTreePosition> &GetFound() const
    {
        return m_found;
    }
...
template <typename T, size_t N>
KdTreeOverlap KdTreeTraversorPointsInRange<T, N>::DeterminOverlap(const Point<T, N> &lwrBound,
                                                                  const Point<T, N> &uprBound) const
{
