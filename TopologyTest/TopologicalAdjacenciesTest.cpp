...
{
    ASSERT_ANY_THROW(
        TopologicalAdjacencies::Create(std::vector<int>{}, std::vector<std::unique_ptr<ITopologicalAdjacency>>{}));
}
...
{
    ASSERT_ANY_THROW(TopologicalAdjacencies::CreateWithPartialCounts(
        Topology::Volume, std::map<TopologyDimension, int>{}, std::vector<std::unique_ptr<ITopologicalAdjacency>>{}));
}
...
TEST(TopologicalAdjacenciesTest, ErrorCreateWithPartialCounts2)
{
    ...
    ASSERT_ANY_THROW(TopologicalAdjacencies::CreateWithPartialCounts(
        Topology::Edge, std::map<TopologyDimension, int>{{Topology::Volume, 1}}, CreateClawAdjacencyList()));
    ASSERT_ANY_THROW(TopologicalAdjacencies::CreateWithPartialCounts(
        Topology::Edge, std::map<TopologyDimension, int>{{Topology::Edge, 1}}, CreateClawAdjacencyList()));
    ASSERT_ANY_THROW(TopologicalAdjacencies::CreateWithPartialCounts(
        Topology::Edge, std::map<TopologyDimension, int>{{Topology::Edge, 1}, {Topology::Corner, 1}},
        CreateClawAdjacencyList()));
    ASSERT_ANY_THROW(TopologicalAdjacencies::CreateWithPartialCounts(
        Topology::Edge, std::map<TopologyDimension, int>{{Topology::Edge, 1}, {Topology::Corner, 0}},
        CreateClawAdjacencyList()));
    ASSERT_ANY_THROW(TopologicalAdjacencies::CreateWithPartialCounts(
        Topology::Edge, std::map<TopologyDimension, int>{{Topology::Edge, 3}, {Topology::Corner, -4}},
        CreateClawAdjacencyList()));
}