...
namespace
{
std::set<const Geometry::IManifoldId *> getBoundaryManifoldIds(
    ...
    const std::vector<const MeshGeneration::IManifoldReconstruction *> &reconstructions)
{
    ...
    str::copy_if(reconstructions, std::back_inserter(result),
                 [&boundaryManifolds](const auto *ptr) { return boundaryManifolds.contains(&(ptr->getManifoldId())); });

    return result;
}

std::vector<EdgeNodesSorted> getBoundaryEdges(
    const std::vector<const MeshGeneration::IManifoldReconstruction *> &boundaryReconstructions)
{
...
std::pair<CellIndex, CellIndex> getInsideAndOutsideCell(
    const EdgeNodesSorted &be, const MeshGeneration::TrianglesNodes &trianglesNodes,
    ...
    const Geometry::IGeometryRegion<MeshGeneration::GeomType, GeomDim2> &region)
{
    ...
    if (isContained0)
        return {connectedTriangles.at(0), connectedTriangles.at(1)};
...
std::pair<std::set<CellIndex>, std::vector<CellIndex>> getBoundaryTriangles(
    const std::vector<EdgeNodesSorted> &boundaryEdges, const MeshGeneration::TrianglesNodes &trianglesNodes,
    const IUniquePointCollection2 &pointCollection,
    const Geometry::IGeometryRegion<MeshGeneration::GeomType, GeomDim2> &region)
{
...
void MeshGeneration::nibble(const Geometry::IGeometryRegion<MeshGeneration::GeomType, GeomDim2> &region,
                            const std::vector<const MeshGeneration::IManifoldReconstruction *> &reconstructions,
                            MeshGeneration::TrianglesNodes &trianglesNodes,
                            const IUniquePointCollection2 &pointCollection, Logger &logger)
{
    ...
    const auto [insideCells, outsideCells] =
        getBoundaryTriangles(boundaryEdges, trianglesNodes, pointCollection, region);
