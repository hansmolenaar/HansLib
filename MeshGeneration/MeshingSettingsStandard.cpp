...
template <size_t N> CellQualityFun<N> *MeshingSettingsStandard<N>::getCellQuality()
{
    return CellQuality2::MinimumAngle;
}

template <size_t N> Logger &MeshingSettingsStandard<N>::getLogger()
{
    return m_logger;
}

template <size_t N> IRefinementPredicateFactory<N> &MeshingSettingsStandard<N>::getRefinementPredicateFactory()
{
    return m_refinementPredicateFactory;
}

template <size_t N> const IGeometryPredicate<double, N> &MeshingSettingsStandard<N>::getGeometryPredicate()
{
    return m_areClose;
}

template <size_t N> const IInitialBoundingboxGenerator<N> &MeshingSettingsStandard<N>::getInitialBbGenerator()
{
