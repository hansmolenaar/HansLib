...
MeshGeneration::Boundary1::Boundary1(const std::vector<Topology::EdgeNodesSorted> &edgeSet,
                                     std::vector<Topology::NodeIndex> activeNodes)
{
...
MeshGeneration::Boundary1 Boundary1::createFromSubSet(const std::vector<Topology::NodeIndex> &activeNodes,
...
Boundary1 Boundary1::createFromBoundaryEdges(const TrianglesNodes &trianglesNodes)
{
    ...
    str::copy_if(trianglesNodes.getAllSortedEdges(), std::back_inserter(activeEdges),
                 [&trianglesNodes](const auto &edge) {
