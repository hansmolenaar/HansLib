...
MeshStatistics MeshStatistics::Create2(const TrianglesNodes &triangles,
                                       const IPointCollection<GeomType, GeomDim2> &points,
                                       CellQualityFun<GeomDim2> *qualityFunction)
{
    return {triangles.getAllNodes().size(), triangles.getAllTriangles().size(),
...
bool MeshStatistics::operator==(const MeshStatistics &other) const
{
    ...
    if (NumberOfCells != other.NumberOfCells)
        return false;
    if (std::abs(Quality - other.Quality) > qualityCrit)
        return false;
    return true;
}