...
{
template <typename TCell> class CellsNodes
{
  public:
    ...
    template <typename C> C &getCommonNodes(CellIndex cellId1, CellIndex cellId2, C &containter) const;

    template <typename C> C &getCellsContainingNodes(C &container, std::span<const Topology::NodeIndex> nodes) const;

    template <typename C> C &getEdgeConnectedNodes(C &container, Topology::NodeIndex node) const;

    template <typename C> C &getAlEdges(C &container) const;
...
template <typename TCell> CellIndex CellsNodes<TCell>::addCell(const TCell &cell)
{
    // TODO add check
    //
...
template <typename TCell> size_t CellsNodes<TCell>::getNumCells() const
{
    return m_cellIdToNodes.size();
}

template <typename TCell> const std::unordered_map<CellIndex, TCell> &CellsNodes<TCell>::getCellIdToNodes() const
{
...
template <typename TCell> void CellsNodes<TCell>::checkNodeId(Topology::NodeIndex nodeId) const
{
...
template <typename TCell> void CellsNodes<TCell>::checkCellId(CellIndex cellId) const
{
...
template <typename TCell> bool CellsNodes<TCell>::isKnownNodeId(Topology::NodeIndex node) const
{
    return m_nodeToCells.find(node) != m_nodeToCells.end();
}

template <typename TCell> bool CellsNodes<TCell>::isKnownCellId(CellIndex cellId) const
{
...
template <typename TCell> bool CellsNodes<TCell>::cellContainsNode(CellIndex cellId, Topology::NodeIndex nodeId) const
{
...
template <typename TCell> void CellsNodes<TCell>::deleteCell(CellIndex cellId)
{
    checkCellId(cellId);
    const auto &nodes = m_cellIdToNodes.at(cellId);
    for (auto node : nodes)
    {
        ...
        if (!found)
        {
            const std::string msg =
                " CellsNodes<TCell>::::deleteCell() inconsistency in m_nodeToCells: " + std::to_string(cellId) + " " +
                std::to_string(node);
            throw MyException(msg);
        }
    }
...
template <typename TCell> std::vector<CellIndex> CellsNodes<TCell>::getAllCellIds() const
{
...
template <typename TCell> std::vector<Topology::NodeIndex> CellsNodes<TCell>::getAllNodes() const
{
    ...
    for (auto iter = m_nodeToCells.begin(); iter != m_nodeToCells.end();
...
C &CellsNodes<TCell>::getCellsContainingNodes(C &result, std::span<const Topology::NodeIndex> nodes) const
{
    ...
    for (auto itr = first; itr != last; ++itr)
    {
        ...
        if (std::all_of(nodes.begin() + 1, nodes.end(),
                        [this, cellId](auto nodeId) { return cellContainsNode(cellId, nodeId); }))
        {
...
template <typename TCell> template <typename C> C &CellsNodes<TCell>::getAlEdges(C &result) const
{
