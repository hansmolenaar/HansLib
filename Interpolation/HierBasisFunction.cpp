...
HierMultiIndex CreateMultiIndex(const std::vector<const IHierBasisFunction1D *> &fies)
{
    ...
    str::transform(fies, std::back_inserter(multiIndex),
                   [](const IHierBasisFunction1D *fie) { return fie->getLevelIndex(); });
...
const IHierBasisFunction *HierBasisFunction_Factory::get(const HierMultiIndex &hmi)
{
    ...
    if (!m_basisFunctions.contains(hmi))
    {
        ...
        str::transform(hmi.get(), std::back_inserter(basisFunctions),
                       [this](const auto &li) { return m_factory1D->get(li); });
