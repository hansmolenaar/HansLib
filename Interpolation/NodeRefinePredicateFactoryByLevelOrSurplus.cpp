...
{
   if (htn->RefinementLevel < m_maxLevel) return true;
   if (std::abs(htn->Surplus) > m_minSurplus) return true;
   return false;
}
...
std::unique_ptr<INodeRefinePredicate> NodeRefinePredicateFactoryByLevelOrSurplus::create(const HierApproximation& approximation)
{
