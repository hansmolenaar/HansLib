...
NodeRefinePredicateFactoryAdaptive::NodeRefinePredicateFactoryAdaptive(double alpha, double fraction)
    : m_alpha(alpha), m_fraction(fraction)
{
}
...
std::unique_ptr<INodeRefinePredicate> NodeRefinePredicateFactoryAdaptive::create(const HierApproximation &approximation)
{
    ...
    str::transform(leafNodes, withRang.begin(), [&approximation](const HierTreeNode *htn) {
        return std::make_pair(htn, approximation(htn->getMultiIndex().toDoubles()));
    });
    ...
    str::transform(nodeToValue, priority.begin(),
                   [](const auto &itr) { return std::make_pair(itr.first, itr.second); });
    ...
    {
        if (std::abs(priority.at(n).second - priority.at(n - 1).second) > 1.0e-10)
            break;
        toRefine.insert(priority.at(n).first);
    }
