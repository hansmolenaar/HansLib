...
HierTreeNode *CreateIfNew(const HierMultiIndex &hmi, int level, const CreateHierNode &creator,
                          std::map<HierMultiIndex, std::unique_ptr<HierTreeNode>> &treeNodeMap)
{
    if (treeNodeMap.contains(hmi))
    {
        if (treeNodeMap.at(hmi)->RefinementLevel != level)
            throw MyException("CreateIfNew");
        return nullptr;
    }
...
std::vector<HierApproximation::RefinementSpecification> GetAllRefinements(
    const HierApproximation &approx, const IHierBasisFunction_Factory &basisFunctionFactory,
    INodeRefinePredicateFactory &refinementFactory)
{
    ...
    for (const auto *leaf : approx.getLeafNodesRO())
    {
        ...
        if (!basisFunctionFactory.canBeRefined(mi))
            continue;
...
HierApproximation::HierApproximation(IHierBasisFunction_Factory &factory,
                                     const IMultiVariableFunctionEvaluate &function)
    : m_function(function), m_factory(factory),
...
std::unique_ptr<HierApproximation> HierApproximation::Create(const IMultiVariableFunctionEvaluate &fie,
                                                             IHierBasisFunction_Factory &factory,
                                                             INodeRefinePredicateFactory &refinementFactory)
{
    std::ostringstream os;
...
std::vector<std::vector<double>> HierApproximation::getCollocationPoints() const
{
    ...
    str::transform(allTreeNodes, result.begin(),
                   [](const HierTreeNode *tn) { return std::vector<double>{tn->getMultiIndex().toDoubles()}; });
    str::sort(result);
    return result;
}
