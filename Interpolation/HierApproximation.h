...
struct HierTreeNode
{
    ...
    const HierMultiIndex &getMultiIndex() const
    {
        return BasisFunction->getMultiIndex();
    }
    size_t getLevel() const
    {
        return getMultiIndex().getL1NormLevel();
    }
};
...
class HierApproximation // : public IMultiVariableRealValuedFunction
{
  public:
    ...
    static std::unique_ptr<HierApproximation> Create(const IMultiVariableFunctionEvaluate &fie,
