...
HierMultiIndex::HierMultiIndex(HierLevelIndex levelIndex) : m_levelsIndices(std::vector<HierLevelIndex>{levelIndex})
{
}
...
{
    return std::accumulate(m_levelsIndices.begin(), m_levelsIndices.end(), static_cast<size_t>(0),
                           [](size_t acc, const HierLevelIndex &add) { return acc + add.getLevel(); });
}
