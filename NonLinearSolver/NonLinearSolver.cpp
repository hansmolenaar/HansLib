...
NonLinearSolver::NonLinearSolver(IRealFunction &problem) : m_problem(problem)
{
    Utilities::MyAssert(problem.GetRangeDimension() == problem.GetDomainDimension());
}

NonLinearSolverStatus NonLinearSolver::Solve(std::span<const double> fx, std::span<double> x,
                                             INonLinearSolverStrategy &strategy)
{
