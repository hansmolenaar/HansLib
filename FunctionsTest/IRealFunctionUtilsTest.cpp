...
TEST(IRealFunctionUtilsTest, TestAdd)
{
    ...
    auto fie0 =
        MultiVariableVectorValuedFunction(std::span<std::shared_ptr<IMultiVariableRealValuedFunction>>(&fie00, 1));

    std::shared_ptr<IMultiVariableRealValuedFunction> fie11(new MultiVariableMonomial({2, 0, 0}));
    auto fie1 =
        MultiVariableVectorValuedFunction(std::span<std::shared_ptr<IMultiVariableRealValuedFunction>>(&fie11, 1));
