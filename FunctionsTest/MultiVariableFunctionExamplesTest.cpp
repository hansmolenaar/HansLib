...
{
    const auto fie = MultiVariableFunctionExamples::DiscontinuousHatProduct(
        std::vector<double>{0.4, 0.6}, std::vector<double>{2, 1}, std::vector<double>{1, 3});
    ...
    // Plotting::MdFunctionsOnUnityToFile("DiscontinuousHatProduct", 2, { [&fie](std::vector<double> x) {return
    // (*fie.Function)(x); } }, 64, std::vector<std::string>{"x", "y", "approx"});
}
...
TEST(MultiVariableFunctionExamplesTest, SumOfSquares)
{
    ...
    // Plotting::MdFunctionsOnUnityToFile("SumOfSquares", 2, { [&fie](std::vector<double> x) {return (*fie.Function)(x);
    // } }, 64, std::vector<std::string>{"x", "y", "approx"});
}
...
TEST(MultiVariableFunctionExamplesTest, SumOfSquares2)
{
    ...
    // Plotting::MdFunctionsOnUnityToFile("SumOfSquares2", 2, { [&fie](std::vector<double> x) {return (*fie)(x); } },
    // 64, std::vector<std::string>{"x", "y", "approx"});
}
