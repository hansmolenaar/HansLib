...
template <typename T> bool operator==(const Interval<T> &lhs, const Interval<T> &rhs)
{
    return lhs.getLower() == rhs.getLower() && lhs.getUpper() == rhs.getUpper();
}

template <typename T> Interval<T>::Interval(const T &val)
{
    m_lower = val;
    m_upper = val;
}

template <typename T> Interval<T>::Interval(const T &val1, const T &val2) : Interval<T>(val1)
{
    add(val2);
}

template <typename T> void Interval<T>::add(const T &val)
{
    m_lower = std::min(m_lower, val);
    m_upper = std::max(m_upper, val);
}

template <typename T> bool Interval<T>::contains(const T &val) const
{
    if (val < m_lower)
        return false;
    return val <= m_upper;
...
template <typename T> bool Interval<T>::contains(const Interval<T> &other) const
{
    return (this->getLower() <= other.getLower()) && (this->getUpper() >= other.getUpper());
}

template <typename T> bool Interval<T>::TryIntersect(const Interval<T> &intv0, const Interval<T> &intv1, T &lo, T &hi)
{
...
template <typename T> T Interval<T>::getMeasure() const
{
    return getUpper() - getLower();
}

template <> inline double Interval<double>::interpolate(double factor) const
{
    return std::lerp(m_lower, m_upper, factor);
}

template <> inline Rational Interval<Rational>::interpolate(Rational factor) const
{
    return m_lower + factor * (m_upper - m_lower);
}

template <> inline double Interval<double>::inverseInterpolate(double arg) const
{
