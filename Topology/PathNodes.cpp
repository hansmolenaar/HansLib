...
Topology::PathNodes::PathNodes(std::span<const NodeIndex> nodes) : m_nodes(nodes.begin(), nodes.end())
{
    if (m_nodes.size() < 2)
        throw MyException("PathNodes::PathNodes too small: size=" + std::to_string(m_nodes.size()));
