...
EdgeNodesDirected::EdgeNodesDirected(NodeIndex n0, NodeIndex n1) : m_nodes{n0, n1}
{
    throwOnDuplicate(m_nodes);
    if (m_nodes[0] == NodeIndexInvalid || m_nodes[1] == NodeIndexInvalid)
    {
        throw MyException("DirectedEdgeNodes not valid: " + std::to_string(m_nodes[0]) + " and " +
                          std::to_string(m_nodes[1]));
    }
}
