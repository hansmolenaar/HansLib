...
TopologicalAdjacencies::TopologicalAdjacencies(std::map<TopologyDimension, int> &&count, AdjacencyMap &&adjacencies)
    : m_count(std::move(count)), m_adjecencies(std::move(adjacencies)),
      m_checkDimension(BoundsCheck<TopologyDimension>::CreateUpperBound(getMaxTopologyDimension()))
{
}

std::unique_ptr<TopologicalAdjacencies> TopologicalAdjacencies::Create(
    const std::vector<int> &count, std::vector<std::unique_ptr<ITopologicalAdjacency>> &&adjacencies)
{
...
std::unique_ptr<TopologicalAdjacencies> TopologicalAdjacencies::CreateWithPartialCounts(
    const TopologyDimension maxDim, std::map<TopologyDimension, int> &&countMap,
    std::vector<std::unique_ptr<ITopologicalAdjacency>> &&adjacencies)
{
    ...
    return std::unique_ptr<TopologicalAdjacencies>(
        new TopologicalAdjacencies(std::move(countMap), std::move(adjacencyMap)));
}
...
{
    if (!m_count.contains(dim))
        return {};
    return m_count.at(dim);
}

std::optional<const ITopologicalAdjacency *> TopologicalAdjacencies::getAdjacency(TopologyDimension dim1,
                                                                                  TopologyDimension dim2) const
{
