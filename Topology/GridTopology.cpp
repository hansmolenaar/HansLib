    ...
    : m_shape(ReferenceShapeHyperCube::Get(static_cast<TopologyDimension>(cellDimensions.size())))
{
    ...
    adjacenciesList.emplace_back(TopologicalAdjacency::Create(maxdim, static_cast<int>(numCells), Corner,
                                                              static_cast<int>(numCorners), cellToCorner));
    m_adjacencies =
        TopologicalAdjacencies::CreateWithPartialCounts(maxdim, std::move(counts), std::move(adjacenciesList));
}
