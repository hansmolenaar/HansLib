...
Topology::CycleNodes::CycleNodes(std::span<const NodeIndex> nodes) : m_nodes(nodes.begin(), nodes.end())
{
    if (m_nodes.size() < 3)
        throw MyException("CycleNodes::CycleNodes too small: size=" + std::to_string(m_nodes.size()));
