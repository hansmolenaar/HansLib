...
template <typename T, size_t N> Point<T, N> Ball<T, N>::getCenter() const
{
    return m_center;
}

template <typename T, size_t N> T Ball<T, N>::getRadius() const
{
    return m_radius;
}

template <typename T, size_t N> T Ball<T, N>::getRadiusSquared() const
{
...
BallPosition Ball<T, N>::getPosition(const Point<T, N> &point, const IGeometryPredicate<T, N> &predicate) const
{
    ...
    if (std::abs(distToCenter2 - m_radiusSquared) <= predicate.getSmallNormSquared())
        return On;
    else if (distToCenter2 < m_radiusSquared)
        return Inside;
    else
        return Outside;
}
...
template <typename T, size_t N>
std::pair<BallPosition, BallPosition> Ball<T, N>::getPositions(const DirectedEdge<T, N> &edge,
                                                               const IGeometryPredicate<T, N> &predicate) const
{
    return {getPosition(edge.point0(), predicate), getPosition(edge.point1(), predicate)};
}

template <typename T, size_t N> BoundingBox<T, N> Ball<T, N>::getBoundingBox() const
{
    std::array<T, N> lwr;
    std::array<T, N> upr;
...
template <typename T, size_t N>
std::optional<Point<T, N>> Ball<T, N>::TryGetFirstIntersectionWithDirectedEdge(
    const Geometry::DirectedEdge<T, N> &edge, const IGeometryPredicate<T, N> &predicate) const
{
    ...
    {
        Utilities::MyAssert(pos1 == BallPosition::Outside);
        if (edge.contains(ip1, predicate) && !predicate.samePoints(ip1, point0))
            return ip1;
        return {};
    }

    Utilities::MyAssert(pos0 == BallPosition::Outside);
    if (predicate.samePoints(point1, ip0))
        return point1;
    if (edge.contains(ip0, predicate))
        return ip0;
    return {};
}
