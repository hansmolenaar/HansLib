...
{
    if (!m_direction.isValid())
        throw MyException("Line<T, N>::Line invalid direction");
}

template <typename T, size_t N>
Line<T, N>::Line(const Point<T, N> &p0, const Point<T, N> &p1) : Line<T, N>::Line(p0, UnitVector<T, N>::Create(p0, p1))
{
}

template <typename T, size_t N> Point<T, N> Line<T, N>::project(const Point<T, N> &point) const
{
