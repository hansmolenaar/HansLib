...
{
   std::array< UnitVector3, NumFacesOnTetrehadron> normal;
   ...
   double minInprod = std::numeric_limits<double>::max();
   for (size_t f0 = 0; f0 < NumNodesOnTetrehadron - 1; ++f0)
   {
      for (size_t f1 = f0 + 1; f1 < NumNodesOnTetrehadron; ++f1)
      {
         minInprod = std::min(minInprod, std::abs((*normal[f0]).innerProduct(normal[f1]->data())));
      }
   }
   return std::acos(minInprod);
}