...
namespace Geometry
{
template <typename T> class DirectedEdge2AsManifold1 : public IManifold1D2<T>
{
  ...
  public:
    ...
    Line<T, GeomDim2> getEuclideanSubspaceAt(const Point<T, GeomDim2> &point,
                                             const IGeometryPredicate<T, GeomDim2> &predicate) const override;

    DirectedEdgeIntersections<T, GeomDim2> getIntersections(
        const DirectedEdge<T, GeomDim2> &edge, const IGeometryPredicate<T, GeomDim2> &predicate) const override;
