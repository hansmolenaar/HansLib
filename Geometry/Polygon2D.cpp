...
template bool Polygon2D::Contains(std::span<const Point<double, 2>> polygon, const Point<double, 2>& point, const IGeometryPredicate<double, 2>& predicate);
...
template<typename T>
bool Polygon2D::Contains(std::span<const Point<T, 2>> polygon, const Point<T, 2>& point, const IGeometryPredicate<T, 2>& predicate)
{
   ...
   for (size_t e = 0; e < polygon.size(); ++e)
   {
      ...
      if ((((p0.at(1) <= point[1]) && (point[1] < p1.at(1))) ||
         ((p1.at(1) <= point[1]) && (point[1] < p0.at(1)))
         ) &&
