...
namespace Geometry
{
template <typename T, size_t N> class DirectedEdgeIntersection
{
  public:
...
template <typename T, size_t N> bool DirectedEdgeIntersection<T, N>::isIsolatedPoint() const
{
    return (std::holds_alternative<DirectedEdgePoint<T, N>>(m_intersection));
}

template <typename T, size_t N> const DirectedEdgePoint<T, N> &DirectedEdgeIntersection<T, N>::getIsolatedPoint() const
{
    if (!isIsolatedPoint())
    {
...
template <typename T, size_t N> const DirectedEdgeInterval<T, N> &DirectedEdgeIntersection<T, N>::getInterval() const
{
    if (isIsolatedPoint())
    {
