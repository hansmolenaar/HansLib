...
template <size_t N>
UniquePointCollectionBinning<N>::UniquePointCollectionBinning(const IGeometryPredicate<double, N> &predicate,
    ...
    : m_predicate(predicate)
{
    for (int n = 0; n < N; ++n)
    {
        ...
        m_bins.emplace_back(
            LocalizationBins::CreateFromValues(values, false, m_predicate.getSmallLengthInDirection(n)));
    }
...
template <size_t N> Point<double, N> UniquePointCollectionBinning<N>::getPoint(PointIndex n) const
{
    return m_points.at(n);
}

template <size_t N> PointIndex UniquePointCollectionBinning<N>::getNumPoints() const
{
...
template <size_t N> const IGeometryPredicate<double, N> &UniquePointCollectionBinning<N>::getGeometryPredicate() const
{
    return m_predicate;
}

template <size_t N> void UniquePointCollectionBinning<N>::deletePoint(PointIndex pointId)
{
    ...
    if (!found)
        throw MyException("UniquePointCollectionBinning<N>::deletePoint() not found: " + std::to_string(pointId));

    m_points.erase(pointId);
}

template <size_t N>
UniquePointCollectionBinning<N>::BinSpecifier UniquePointCollectionBinning<N>::locate(
...
template <size_t N> PointIndex UniquePointCollectionBinning<N>::addIfNew(const Point<double, N> &point)
{
...
template <size_t N> const LocalizationBins &UniquePointCollectionBinning<N>::getBins(int direction) const
{
    return m_bins.at(direction);
}

template <size_t N>
std::optional<PointIndex> UniquePointCollectionBinning<N>::tryGetClosePointInBin(const Point<double, N> &p,
                                                                                 const BinSpecifier &bins) const
{
    const auto [first, last] = m_pointsInBin.equal_range(bins);
    for (auto itr = first; itr != last; ++itr)
    {
