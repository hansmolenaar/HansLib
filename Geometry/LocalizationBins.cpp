...
LocalizationBins::LocalizationBins(std::vector<double> vertices) : m_vertices(std::move(vertices))
{
}
...
{
    if (nBins < 1)
        throw MyException("LocalizationBins::CreateUniform need at least 1 bin");
    ...
    if (binSize <= 0.0)
        throw MyException("LocalizationBins::CreateUniform incorrect bin size");
...
{
    if (minBinSize <= 0)
        throw MyException("LocalizationBins::CreateFromValues minBinSize must be positive");
    if (values.size() < 2)
        throw MyException("LocalizationBins::CreateFromValues need at least 2 values");
    str::sort(values);
    ...
    if (!strict)
        vertices.push_back(std::numeric_limits<double>::lowest());
    ...
    if (!strict)
        vertices.push_back(std::numeric_limits<double>::max());
...
{
    if (x < m_vertices.front())
        throw MyException("LocalizationBins::find value below lower bound");
    ...
    if (x > m_vertices.back())
        throw MyException("LocalizationBins::find value above upper bound");
    // x == m_vertices.back()
...
std::string LocalizationBins::toString() const
{
    ...
    for (size_t n = first; n < last; ++n)
        binSizes.push_back(m_vertices.at(n + 1) - m_vertices.at(n));
    const ArrayStatistics<double> stats(binSizes);

    std::ostringstream oss;
    oss << (skipLower ? "(-INF)" + sep : "") << "LWR=" << m_vertices.at(first) << sep << "UPR=" << m_vertices.at(last)
        << (skipUpper ? sep + "(+INF)" : "") << sep << stats;
    return oss.str();
}