...
template <typename T> const std::string &Polygon2AsRegion<T>::getName() const
{
    return m_name;
}

template <typename T> BoundingBox<T, GeomDim2> Polygon2AsRegion<T>::getBoundingBox() const
{
    return m_bb;
}

template <typename T>
bool Polygon2AsRegion<T>::contains(const Point<T, GeomDim2> &point,
                                   const IGeometryPredicate<T, GeomDim2> &predicate) const
{
    return Polygon2D::Contains<T>(m_polygon, point, predicate);
}

template <typename T> const IRegionManifolds<T, GeomDim2> &Polygon2AsRegion<T>::getManifolds() const
{
    return *this;
}

template <typename T> std::vector<const IManifold<T, GeomDim2> *> Polygon2AsRegion<T>::getBoundaryHyperManifolds() const
{
...
template <typename T> std::vector<const IManifold<T, GeomDim2> *> Polygon2AsRegion<T>::getAllManifolds() const
{
...
template <typename T>
std::vector<const IManifold<T, GeomDim2> *> Polygon2AsRegion<T>::getConnectedHighers(
    const IManifold<T, GeomDim2> &manifold) const
{
    const auto numPoints = m_polygon.size();
    const auto predicateManifold = [&manifold](const Manifold0<T, GeomDim2> &pointManifold) {
        return &pointManifold == &manifold;
    };
    const auto found = str::find_if(m_pointManifolds, predicateManifold);
    if (found == m_pointManifolds.end())
        return {};
...
template <typename T>
std::vector<const IManifold<T, GeomDim2> *> Polygon2AsRegion<T>::getConnectedLowers(
    const IManifold<T, GeomDim2> &manifold) const
{
    const auto numPoints = m_polygon.size();
    const auto predicateManifold = [&manifold](const DirectedEdge2AsManifold1<T> &edgeManifold) {
        return &edgeManifold == &manifold;
    };
    const auto found = str::find_if(m_edgeManifolds, predicateManifold);
    if (found == m_edgeManifolds.end())
        return {};
