...
ML::AnnDataSet::AnnDataSet(size_t sizeIn, size_t sizeOut)
    : m_sizeIn(sizeIn), m_sizeOut(sizeOut), m_sizeInOut(sizeIn + sizeOut)
{
}

ML::AnnDataSet::AnnDataSet(std::span<const double> input, std::span<const double> output)
    : AnnDataSet(input.size(), output.size())
{
