...
void ML::AnnWeightsSingleBias::setActivation(std::span<const double> outputPrv, std::span<const double> params,
                                             std::span<double> activation) const
{
    Utilities::MyAssert(params.size() == getNumberOfParameters());
    m_matrixOnly.setActivation(outputPrv, std::span<const double>(params.begin(), m_matrixOnly.getNumberOfParameters()),
                               activation);
    const double bias = params.back();
    std::transform(activation.begin(), activation.end(), activation.begin(), [bias](double x) { return x + bias; });
}

void ML::AnnWeightsSingleBias::backpropagateError(std::span<const double> errorCur, std::span<const double> params,
                                                  std::span<double> errorPrv) const
{
    m_matrixOnly.backpropagateError(
        errorCur, std::span<const double>(params.begin(), m_matrixOnly.getNumberOfParameters()), errorPrv);
}

void ML::AnnWeightsSingleBias::backpropagateParamDeriv(std::span<const double> errorCur,
                                                       std::span<const double> outputPrv,
                                                       std::span<double> dCost_dParam) const
{
    m_matrixOnly.backpropagateParamDeriv(errorCur, outputPrv,
                                         std::span<double>(dCost_dParam.begin(), m_matrixOnly.getNumberOfParameters()));
