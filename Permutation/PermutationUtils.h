...
template <typename Distance> static bool isIsometry(const Permutation &permutation, Distance distance)
{
...
template <typename T> Permutation::Entry SafeCastToEntry(T number)
{
    if (number < 0)
        throw MyException("CastToEntry negative number " + std::to_string(number));
    if (std::cmp_greater_equal(number, std::numeric_limits<Permutation::Entry>::max()))
        throw MyException("CastToEntry too large number");
