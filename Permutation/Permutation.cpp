...
Permutation::Permutation(std::vector<Entry> &&permut) : m_permut(std::move(permut)), m_cycles(GetCycles(m_permut))
{
...
Permutation Permutation::CreateFromCycle(Entry cardinality, std::span<const Entry> cycle)
{
    ...
    {
        if (permut.at(n) == InvalidEntry)
            permut.at(n) = n;
    }
    return Permutation::Create(permut);
}

Permutation Permutation::CreateFromDisjunctCycles(Entry cardinality,
                                                  std::initializer_list<std::initializer_list<Entry>> cycles)
{
    ...
    for (const auto &cycle : cycles)
    {
        ...
        if (*minmax.min < 0)
            throw MyException("CreateFromDisjunctCycles min element out of range: " + std::to_string(*minmax.min));
        if (*minmax.max >= cardinality)
            throw MyException("CreateFromDisjunctCycles max element out of range: " + std::to_string(*minmax.max));
        Entry prev = InvalidEntry;
        for (int c : cycle)
        {
            if (prev != InvalidEntry)
            {
                if (perm[prev] != InvalidEntry)
                    throw MyException("Permutation::CreateFromDisjunctCycles duplicate entry!");
                perm[prev] = c;
            }
            prev = c;
        }
...
{
    if (pow == 0)
        return Permutation::CreateTrivial(getCardinality());
    if (pow == 1)
        return *this;
    if (pow < 0)
        return getInverse().getPower(-pow);
...
Permutation::Parity Permutation::getParity() const
{
    const auto sum = std::accumulate(m_cycles.begin(), m_cycles.end(), static_cast<Permutation::Entry>(0),
                                     [](Permutation::Entry init, const std::vector<Permutation::Entry> &perm) {
                                         return init + PermutationUtils::SafeCastToEntry(perm.size() + 1);
                                     });
