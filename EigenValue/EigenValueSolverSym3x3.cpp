...
int EigenValueSolverSym3x3Utils::FunctionP3Inv::GetDomainDimension() const
{
    return 6;
}
...
double EigenValueSolverSym3x3Utils::DetAMinQI::Evaluate(std::span<const double> x) const
{
    ...
    const double result =
        bmat[0] * bmat[1] * bmat[2] + MathConstants::SQRT1_2 * bmat[3] * bmat[4] * bmat[5] -
...
void EigenValueSolverSym3x3Utils::DetAMinQI::Derivative(std::span<const double> x, std::span<double> dfdx) const
{
    ...
    std::array<ComponentAMinQI, 6> bcomps = {ComponentAMinQI(0), ComponentAMinQI(1), ComponentAMinQI(2),
                                             ComponentAMinQI(3), ComponentAMinQI(4), ComponentAMinQI(5)};
...
EigenValueSolverSym3x3Utils::FunctionDetB::FunctionDetB(const IMultiVariableRealValuedFunction &p) : m_p(p)
{
