...
template <typename T, size_t N> class UnitVector
{
  public:
    UnitVector() = default;
    bool isValid() const
    ...
    // inner product
    friend T operator*(const UnitVector<T, N> &lhs, const UnitVector<T, N> &rhs)
    {
        return PointUtils::innerProduct(lhs.data(), rhs.data());
    };
...
template <typename T, size_t N> UnitVector<T, N>::UnitVector(std::array<T, N> &&unitVector) : m_vector(unitVector)
{
}

template <typename T, size_t N> T UnitVector<T, N>::operator[](int d) const
{
...
template <typename T, size_t N> UnitVector<T, N> UnitVector<T, N>::Create(const Point<T, N> &point)
{
    return Create(std::span<const T>(point.begin(), point.end()));
}

template <typename T, size_t N> UnitVector<T, N> UnitVector<T, N>::Create(std::span<const T> cors)
{
...
template <typename T, size_t N> Point<T, N> operator*(T factor, const UnitVector<T, N> &uv)
{
    return uv * factor;
}

template <typename T, size_t N> T UnitVector<T, N>::innerProduct(const Point<T, N> &p) const
{
    return std::inner_product(m_vector->begin(), m_vector->end(), p.begin(), 0.0);
}

template <typename T, size_t N> UnitVector<T, N> UnitVector<T, N>::getNormalTo(Point<T, N> v)
{
    static_assert(N == 2);
    std::swap(v[0], v[1]);
    v[0] = -v[0];
