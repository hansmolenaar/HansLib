...
template <class T, size_t N> using Point = std::array<T, N>;
...
template <typename T, size_t N> std::array<T, N> operator+(std::array<T, N> lhs, const std::array<T, N> &rhs)
{
...
template <typename T> std::array<T, 3> operator*(const std::array<T, 3> &a, const std::array<T, 3> &b)
{
...
template <typename T, size_t N> std::array<T, N> operator*(T factor, std::array<T, N> result)
{
...
template <typename T, size_t N> std::ostream &operator<<(std::ostream &stream, const Point<T, N> &point)
{
...
{
template <typename T, size_t N> T GetNormSquared(const Point<T, N> &point)
{
    return std::inner_product(point.begin(), point.end(), point.begin(), T(0));
}

template <typename T, size_t N> T GetDistanceSquared(const Point<T, N> &point0, const Point<T, N> &point1)
{
    const auto dif = point0 - point1;
    return GetNormSquared(dif);
}

template <typename T, size_t N> T GetL1Distance(const Point<T, N> &point0, const Point<T, N> &point1)
{
    const Point<T, N> dif = point0 - point1;
    return std::accumulate(dif.begin(), dif.end(), T(0), [](T sum, T val) { return sum + std::abs(val); });
}

template <typename T, size_t N> T Angle(const Point<T, N> &p0, const Point<T, N> &p1, const Point<T, N> &p2)
{
    const auto d0 = p1 - p0;
    const auto d2 = p1 - p2;
    ...
    if (norm02 < eps)
        throw MyException("Point::Angle degenerated");
    return std::acos(inprod / norm02);
}

template <typename T, size_t N> T innerProduct(const Point<T, N> &p0, const Point<T, N> &p1)
{
