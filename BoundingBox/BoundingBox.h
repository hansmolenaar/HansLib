...
template <typename T, size_t N> class BoundingBox
{
  public:
    static BoundingBox Create(const std::span<const T> &);

    template <typename Container> static BoundingBox CreateFromList(const Container &container);

    static BoundingBox CreateFrom2Points(const Point<T, N> &point1, const Point<T, N> &point2);

    template <typename TScale> BoundingBox<T, N> scaleFrom01(const BoundingBox<TScale, N> &scale) const;

    template <typename TScale> Point<T, N> scaleFromPoint01(const Point<TScale, N> &scale) const;

    template <typename Container, typename F>
    static BoundingBox CreateFromListTransformed(const Container &container, F fun);

    const Interval<T> &getInterval(int n) const
    {
        return m_intervals.at(n);
    }
    std::span<const Interval<T>> getIntervals() const
    {
        return m_intervals;
    }
...
template <typename T, size_t N> BoundingBox<T, N> BoundingBox<T, N>::Create(const std::span<const T> &values)
...
template <typename T, size_t N> T BoundingBox<T, N>::getLower(int d) const
{
    return m_intervals.at(d).getLower();
}

template <typename T, size_t N> T BoundingBox<T, N>::getUpper(int d) const
{
...
template <typename T, size_t N>
std::optional<BoundingBox<T, N>> BoundingBox<T, N>::TryGetOverlap(const BoundingBox<T, N> &bb1,
                                                                  const BoundingBox<T, N> &bb2)
{
    std::array<T, N> lwr;
    std::array<T, N> upr;
    for (int n = 0; n < N; ++n)
...
template <typename T, size_t N> T BoundingBox<T, N>::getLengthDiagonalSquared() const
{
