...
template<int N>
void Test_IFinitePointGroupAction(const IFinitePointGroupAction<N>& groupAction)
{
   ...
   const auto randomPoint = GetRandomPoint<N>();
   for (auto elm1 : groupElements)
   {
      for (auto elm2 : groupElements)
      {
         const auto elm3 = group(elm1, elm2);
         const auto point3 = groupAction(elm3, randomPoint);

         const auto point2 = groupAction(elm2, randomPoint);
