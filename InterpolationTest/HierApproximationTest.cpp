...
std::shared_ptr<IMultiVariableRealValuedFunction> GetSomePolynomial()
{
    const std::vector<std::pair<std::vector<int>, double>> terms{
        std::make_pair(std::vector<int>{1, 2}, 1.0), std::make_pair(std::vector<int>{1, 4}, -1.0),
        std::make_pair(std::vector<int>{2, 2}, -1.0), std::make_pair(std::vector<int>{2, 4}, 1.0)};
...
TEST(HierApproximationTest, GetSomePolynomial_1)
{
    ...
    // Plotting::MdFunctionsOnUnityToFile("HierBasisFunctionTest_GetSomePolynomial_", 2, { [&fie](std::vector<double> x)
    // {return (*fie)(x); } }, 20, std::vector<std::string>{"x", "y", "f"});
    ...
    // Plotting::MdFunctionsOnUnityToFile("HierBasisFunctionTest_GetSomePolynomial_Approximation", 2, {
    // [&approximation](std::vector<double> x) {return (*approximation)(x); } }, 20, std::vector<std::string>{"x", "y",
    // "approx"});
}
...
TEST(HierApproximationTest, GetSomePolynomial_3)
{
    ...
    str::transform(approximation->getAllTreeNodesRO(), std::back_inserter(indices),
                   [](const HierTreeNode *hn) { return hn->getMultiIndex(); });
    str::sort(indices);
...
TEST(HierApproximationTest, Linear)
{
    ...
    // 2*x + 3*y
    const auto linear = MultiVariableFunctionExamples::GetPolynomial(
        std::vector<std::pair<std::vector<int>, double>>{{std::vector<int>{1, 0}, 2.0}, {std::vector<int>{0, 1}, 3.0}});
...
TEST(HierApproximationTest, CamelBack)
{
    ...
    auto scaled = std::make_unique<MultiVariableFunctionEvaluateFrom01>(
        std::move(flipped), std::vector<Interval<double>>{Interval<double>(-3, 3), Interval<double>(-2, 2)});
    ...
    // Plotting::MdFunctionsOnUnityToFile("HierBasisFunctionTest_CamelBack", 2, { [&approximation](std::vector<double>
    // x) {return (*approximation)(x); } }, 64, std::vector<std::string>{"x", "y", "approx"});
}
...
TEST(HierApproximationTest, SquaredHat)
{
    ...
    str::transform(str::iota_view(size_t{0}, dimension), pos.begin(),
                   [](size_t n) { return 1.0 / std::sqrt(n + 2.0); });
    ...
    // Plotting::MdFunctionsOnUnityToFile("HierBasisFunctionTest_SquaredHat", 2, { [&squaredHat](std::vector<double> x)
    // {return (*squaredHat.Function)(x); } }, 64, std::vector<std::string>{"x", "y", "approx"});
}
