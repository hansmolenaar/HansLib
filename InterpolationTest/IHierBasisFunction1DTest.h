...
inline void IHierBasisFunction1DTest_Interface(const IHierBasisFunction1D &fie)
{
    ...
    const double fieL = fie.Evaluate(support.getLower());
    const double fieH = fie.Evaluate(support.getUpper());
    if (std::abs(fieL) + std::abs(fieH) < epsilon)
    {
        const double fieM = fie.Evaluate(std::midpoint(support.getLower(), support.getUpper()));
