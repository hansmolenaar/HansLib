...
TEST(LagrangeInterpolationTest, Runge5)
{
    ...
    // const auto& plot = Plotting::PlotFunction({ [&rungeFie](double x) {return rungeFie->Evaluate(x); }, [&li](double
    // x) {return li.Evaluate(x); } }, -1, 1, 100);
}
...
TEST(LagrangeInterpolationTest, SkewedHat)
{
    ...
    const auto values =
        Plotting::EvaluateFunctions({[&hatFie](double x) { return hatFie->Evaluate(x); },
