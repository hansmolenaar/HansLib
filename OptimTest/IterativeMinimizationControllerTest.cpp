...
{
  public:
    FakeIterativeMinimizationStep(IterativeMinimizationStep status, const IMultiVariableFunctionEvaluate &fie)
        : m_stepResult(status), m_function(fie)
    {
    }
    ...
    const IMultiVariableFunctionEvaluate &getObjectiveFunction() const
    {
        return m_function;
    }
...
TEST(IterativeMinimizationControllerTest, CheckLogic)
{
    ...
    // StepSucces
    {
        ...
        ASSERT_EQ(IterativeMinimizationController::Iterate(iterator, critConverged).Status, Converged);
        // Does not terminate ASSERT_EQ(IterativeMinimizationController::Iterate(iterator, critNotConverged).Status,
        // NotConverged);
        ASSERT_EQ(IterativeMinimizationController::Iterate(iterator, critMaxIter).Status, MaxIterExceeded);
        ASSERT_EQ(IterativeMinimizationController::Iterate(iterator, critFail).Status, Fail);
    }
...
{
    std::shared_ptr<IMultiVariableFunctionEvaluate> fiePtr = MultiVariableFunctionExamples::GetPolynomial(
        std::vector<std::pair<std::vector<int>, double>>{{std::vector<int>{2}, 1.0}});
