...
{
    if (index < 0)
        throw MyException("FieldInfoStatic::CheckRowColIndex index should be >= 0, actual " + std::to_string(index));
    if (index >= NumRowColBox)
        throw MyException("FieldInfoStatic::CheckRowColIndex index should be < 9, actual " + std::to_string(index));
}

void FieldInfoStatic::CheckValue(Value value)
{
    if (value < 1)
        throw MyException("FieldInfoStatic::CheckValue value should be >= 1, actual " + std::to_string(value));
    if (value > NumValues)
        throw MyException("FieldInfoStatic::CheckValue value should be <= 9, actual " + std::to_string(value));
}
...
{
    if (type == RowColBoxType::Row)
        return GetRow(subSetIndex);
    if (type == RowColBoxType::Col)
        return GetCol(subSetIndex);
    if (type == RowColBoxType::Box)
        return GetBox(subSetIndex);
...
const ConnectedFields &FieldInfoStatic::GetSortedConnectedFields(FieldIndex field)
{
    ...
    if (s_instance.empty())
    {
        ...
        for (FieldIndex f : getAllFields())
        {
            ...
            if (sameRowColBox.size() != NumConnectedFields)
                throw MyException("FieldInfoStatic::getConnectedFields unexepcted number of connections: " +
                                  std::to_string(sameRowColBox.size()));
...
ActiveFields FieldInfoStatic::GetCommonConnectedFields(FieldIndex field1, FieldIndex field2)
{
    ...
    ActiveFields result;
    str::copy_if(common, std::back_inserter(result),
                 [field1, field2](FieldIndex field) { return field != field1 && field != field2; });
    return result;
}
...
{
    if (field1 == field2)
        throw MyException("FieldInfoStatic::AreConnected self connection?");
...
RowColBoxIndex FieldInfoStatic::FieldToRowColBox(RowColBoxType type, FieldIndex field)
{
    ...
    if (type == RowColBoxType::Col)
        return FieldToCol(field);
    if (type == RowColBoxType::Box)
        return FieldToBox(field);
