...
{
    ActiveFields result;
    str::copy_if(FieldInfoStatic::getAllFields(), std::back_inserter(result), [&potentials, value](FieldIndex f) {
        return potentials.get(f).count() == 2 && potentials.get(f).containsValue(value);
    });
    return result;
}

std::vector<std::pair<FieldIndex, FieldIndex>> SolverSweepBiValueLinksSingleValue::GetBiValueAdjecencies(
    const Potentials &potentials, Value value)
{
...
Value SolverSweepBiValueLinksSingleValue::getOtherValue(const Potentials &potentials, FieldIndex field) const
{
    ...
    if (potentialPair.front() == m_value)
        return potentialPair.back();
    if (potentialPair.back() == m_value)
        return potentialPair.front();
