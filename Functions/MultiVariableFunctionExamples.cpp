    ...
    const std::vector<double> &pos)
{
   ...
   auto function =
       std::make_shared<MultiVariableRealValuedFunctionNoDerivatives>(dim, std::make_shared<SkewedHatSquaredEval>(pos));
    ...
    const std::vector<double> &xpeak, const std::vector<double> &ypeakLeft, const std::vector<double> &ypeakRight)
{
   ...
   for (size_t n = 0; n < xpeak.size(); ++n)
   {
      functions.emplace_back(
          SingleVariableFunctionExamples::GetDiscontinuousHatFunction(xpeak.at(n), ypeakLeft.at(n), ypeakRight.at(n)));
   }
   Examplefunction result;
   auto function =
