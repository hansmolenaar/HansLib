...
std::shared_ptr<SingleVariableFunctionOnInterval> SingleVariableFunctionOnInterval::CreateLinearOnInterval(const Interval<double> intv, double valL, double valR)
{
   const double factor = (valR - valL) / intv.getMeasure();
   auto poly = SingleVariablePolynomial::Create(std::vector<std::tuple<double, int>>{std::make_tuple(factor, 1), std::make_tuple(valL - intv.getLower()*factor, 0)});
