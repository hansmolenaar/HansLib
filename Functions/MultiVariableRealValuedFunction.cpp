...
MultiVariableRealValuedFunction::MultiVariableRealValuedFunction(
    std::vector<bool> isactive, std::function<double(std::span<const double>)> function,
...
MultiVariableRealValuedFunction MultiVariableRealValuedFunction::CreateNull(int numVar)
{
    ...
    return MultiVariableRealValuedFunction(
        active, [](std::span<const double>) { return 0.0; },
...
MultiVariableRealValuedFunction operator-(const IMultiVariableRealValuedFunction &f,
                                          const IMultiVariableRealValuedFunction &g)
{
    ...
    std::function<void(std::span<const double>, std::span<double>)> deriv = [&](std::span<const double> x,
                                                                                std::span<double> df) {
...
MultiVariableRealValuedFunction operator/(const IMultiVariableRealValuedFunction &numer,
                                          const IMultiVariableRealValuedFunction &denom)
{
