...
template <typename T> MultiIndex<T> MultiIndex<T>::Create(std::span<const T> dimensions)
{
...
template <typename T> size_t MultiIndex<T>::getFlatSize() const
{
    return m_flatLength;
}

template <typename T> T MultiIndex<T>::at(size_t n) const
{
