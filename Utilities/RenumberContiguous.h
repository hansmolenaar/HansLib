...
namespace Utilities
{
template <class Tin, class Tcontiguous> class RenumberContiguous
{
  public:
    template <class FwdItr> RenumberContiguous(FwdItr first, FwdItr last);

    std::optional<Tcontiguous> toContiguous(const Tin &value) const;
    const Tin &at(Tcontiguous idx) const;
    Tcontiguous size() const
    {
        return m_toContiguous.size();
    }
...
template <class Tin, class Tcontiguous> const Tin &RenumberContiguous<Tin, Tcontiguous>::at(Tcontiguous idx) const
{
