    ...
    const std::vector<const MeshGeneration::IManifoldReconstruction *> &reconstructions)
{
    ...
    str::transform(reconstructions, manifolds1.begin(),
                   [](const auto *ptr) { return dynamic_cast<const Manifold1Reconstruction *>(ptr); });
    std::erase(manifolds1, nullptr);

    for (const auto &edge : boundaryEdges)
    {
        const EdgeNodesSorted edgeSorted(edge[0], edge[1]);
        ASSERT_TRUE(str::any_of(
            manifolds1, [&edgeSorted](const Manifold1Reconstruction *mptr) { return mptr->contains(edgeSorted); }));
    }
}

void checkMesh(const IGeometryRegion<double, GeomDim2> &region, const TrianglesNodes &triangles,
               const std::vector<const MeshGeneration::IManifoldReconstruction *> &reconstructions,
               const IPointCollection2 &points, IMeshingSettings2 &settings)
{
    IRegionManifoldsTestInterface(region.getManifolds(), settings.getGeometryPredicate());
    checkTriangleArea(triangles, points);
    const bool succes =
        MeshGeneration2::checkReconstructions(region.getManifolds(), reconstructions, settings.getLogger());
    ASSERT_TRUE(succes);
...
TEST(MeshGeneration2Test, SingleTriangleToWorld)
{
    ...
    Mesh2 mesh;
    MeshGeneration2::BaseTriangulationToWorld(baseTriangles, areClose, bb, mesh.createPoints(), mesh.getTriangles(),
                                              logger);
...
TEST(MeshGeneration2Test, Ball2)
{
    ...
    MeshGeneration2::BaseTriangulationToWorld(mesh.getBaseTriangles(), settings.getGeometryPredicate(), bbInitial,
                                              mesh.createPoints(), mesh.getTriangles(), settings.getLogger());
...
TEST(MeshGeneration2Test, Sphere2AndEdge)
{
    ...
    const auto cellId = trianglesNodes.addTriangle(TriangleNodesOriented(
        points.addIfNew(Point2{-1, -2}), points.addIfNew(Point2{1, -2}), points.addIfNew(Point2{0, -0.9})));
...
TEST(MeshGeneration2Test, Sphere2AndEdge_immobilePoint)
{
    ...
    const Manifold0<GeomType, GeomDim2> pointManifold(Point2{0, -1},
                                                      "MeshGeneration2Test_Sphere2AndEdge_immobilePoint");
    const Sphere2AsManifold1<GeomType> manifold(sphere);
    const PointClose<GeomType, GeomDim2> predicate;
    UniquePointCollectionBinning<GeomDim2> points(predicate, std::vector<Point2>{bb.getLower(), bb.getUpper()});
    ManifoldsAndNodes<GeomDim2> manifoldsAndNodes;
    ...
    ASSERT_MYEXCEPTION_MESSAGE(AddEdgeManifold1Intersections(manifold, edge, trianglesNodes, manifoldsAndNodes, points),
                               "UniquePointCollectionBinning<N>::movePoint point already exists");
}
...
TEST(MeshGeneration2Test, Sphere2_intersect_4)
{
    ...
    const auto manifoldPtr = dynamic_cast<const Geometry::IManifold1D2<GeomType> *>(
        Utilities::Single(ballAsRegion.getManifolds().getAllManifolds()));

    MeshGeneration2::AddManifold1Intersections(*manifoldPtr, mesh.getTriangles(), mesh.setManifoldsAndNodes(),
                                               mesh.getSetPoints(), settings.getLogger());
    ...
    mesh.addReconstructions(MeshGeneration2::createReconstructions(ballAsRegion.getManifolds(), mesh.getTriangles(),
                                                                   mesh.getManifoldsAndNodes(), mesh.getPoints()));

    ASSERT_EQ(mesh.getReconstructions().size(), 1);
    const auto reconstruction =
        dynamic_cast<const Manifold1Reconstruction &>(*mesh.getReconstructions().front()).getReconstruction();
...
TEST(MeshGeneration2Test, Triangle2_1)
{
    ...
    const auto polygonPoints = generateRegularPolygon(3, 0.1);
    ...
    MeshGeneration2::AddAllManifolds0(manifolds0, mesh.getTriangles(), mesh.setManifoldsAndNodes(), mesh.getSetPoints(),
                                      settings.getLogger());

    for (const auto *manifold1 : region.getManifoldsOfType<const IManifold1D2<GeomType> *>())
    {
        MeshGeneration2::AddManifold1Intersections(*manifold1, mesh.getTriangles(), mesh.setManifoldsAndNodes(),
                                                   mesh.getSetPoints(), settings.getLogger());
    }

    mesh.addReconstructions(MeshGeneration2::createReconstructions(region, mesh.getTriangles(),
                                                                   mesh.getManifoldsAndNodes(), mesh.getPoints()));
...
TEST(MeshGeneration2Test, Triangle2_1_0)
{
    ...
    const auto polygonPoints = generateRegularPolygon(3, angle);
    const Polygon2AsRegion<double> region(polygonPoints, "triangle");
    ...
    nibble(region, mesh.getReconstructions(), mesh.getTriangles(), mesh.getPoints(), settings.getLogger());
    ASSERT_EQ(mesh.getTriangles().getAllTriangles().size(), 154);
    checkMesh(region, mesh.getTriangles(), mesh.getReconstructions(), mesh.getPoints(), settings);
