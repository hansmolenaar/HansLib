...
ActionDihedral::ActionDihedral(int numVertices, const Point2 &point) : m_group(DihedralGroup::Create(numVertices))
{
    if (numVertices < 1)
        throw MyException("ActionDihedral::ActionDihedral unexpected size " + std::to_string(numVertices));
    ...
    if (!uv.isValid())
        throw MyException("ActionDihedral::ActionDihedral 0 is not a valid reference point");
    for (int n = 0; n < numVertices; ++n)
    {
        const double angle = n * std::numbers::pi / numVertices;
        auto rotated = Rotate2D(angle)(uv);
        std::unique_ptr<IPointTransformation<double, 2>> plane =
            std::make_unique<Hyperplane<double, 2>>(Point2{}, std::move(rotated));
