...
DihedralGroup::DihedralGroup(std::unique_ptr<IFiniteGroup> &groupTable) : m_groupTable(std::move(groupTable))
{
}
...
std::unique_ptr<IFiniteGroup> DihedralGroup::Create(GroupElement numVertices)
{
    ...
    std::unique_ptr<IIndexer<GroupElement>> indexer =
        std::make_unique<IndexerRowMajor<GroupElement>>(tableDim, tableDim);
