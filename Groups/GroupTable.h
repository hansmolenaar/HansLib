...
class GroupTable : public IFiniteGroup
{
  public:
    ...
    static std::unique_ptr<IFiniteGroup> CreateUsingBinaryOperator(const std::vector<T> &, BinOp binop,
                                                                   bool checkAssociativity);

    static std::unique_ptr<IFiniteGroup> Create(std::unique_ptr<IIndexer<GroupElement>> &,
                                                const std::vector<GroupElement> &, bool checkAssociativity = true);
    static std::pair<std::unique_ptr<IFiniteGroup>, std::vector<Permutation>> GeneratedBy(
...
template <typename T, typename BinOp>
std::unique_ptr<IFiniteGroup> GroupTable::CreateUsingBinaryOperator(const std::vector<T> &elements, BinOp binop,
                                                                    bool checkAssociativity)
{
    std::unique_ptr<IIndexer<GroupElement>> indexer;
    if (elements.empty())
        return Create(indexer, std::vector<GroupElement>{});

    if (elements.size() >= std::numeric_limits<GroupElement>::max())
        throw MyException("CreateUsingBinaryOperator too large: " + std::to_string(elements.size()));
