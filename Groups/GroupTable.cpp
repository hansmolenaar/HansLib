...
std::unique_ptr<IFiniteGroup> GroupTable::Create(std::unique_ptr<IIndexer<GroupElement>> &indexer,
                                                 const std::vector<GroupElement> &elements, bool checkAssociativity)
{
...
std::pair<std::unique_ptr<IFiniteGroup>, std::vector<Permutation>> GroupTable::GeneratedBy(
    const std::vector<Permutation> &permutationsIn)
{
    std::unique_ptr<IIndexer<GroupElement>> indexer;
    if (permutationsIn.empty())
        return {Create(indexer, std::vector<GroupElement>{}), std::vector<Permutation>{}};
    ...
    return {CreateUsingBinaryOperator<Permutation, decltype(Permutation::compose)>(permutations, Permutation::compose,
                                                                                   checkAssociativity),
            permutations};
}
