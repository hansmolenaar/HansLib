...
const std::array<std::array<bool, nrDims>, CubeGroup::numVertices> Coordinates{
    std::array<bool, nrDims>{false, false, false}, std::array<bool, nrDims>{true, false, false},
    std::array<bool, nrDims>{false, true, false},  std::array<bool, nrDims>{true, true, false},

    std::array<bool, nrDims>{false, false, true},  std::array<bool, nrDims>{true, false, true},
    std::array<bool, nrDims>{false, true, true},   std::array<bool, nrDims>{true, true, true},
};
...
std::vector<Permutation> Symmetries = {
    ...
    Permutation::CreateFromDisjunctCycles(CubeGroup::numVertices,
                                          {{1, 7}, {3, 5}, {0, 6}, {2, 4}}), // i axis 4-fold, pi
    Permutation::CreateFromDisjunctCycles(CubeGroup::numVertices,
                                          {{1, 5, 7, 3}, {0, 4, 6, 2}}), // i axis 4-fold, 3*pi/2

    Permutation::CreateFromDisjunctCycles(CubeGroup::numVertices, {{0, 1, 5, 4}, {2, 3, 7, 6}}), // j axis 4-fold, pi/2
    Permutation::CreateFromDisjunctCycles(CubeGroup::numVertices,
                                          {{0, 5}, {1, 4}, {2, 7}, {3, 6}}), // j axis 4-fold, pi
    Permutation::CreateFromDisjunctCycles(CubeGroup::numVertices,
                                          {{0, 4, 5, 1}, {2, 6, 7, 3}}), // j axis 4-fold, 3*pi/2

    Permutation::CreateFromDisjunctCycles(CubeGroup::numVertices, {{0, 1, 3, 2}, {4, 5, 7, 6}}), // j axis 4-fold, pi/2
    Permutation::CreateFromDisjunctCycles(CubeGroup::numVertices,
                                          {{0, 3}, {1, 2}, {4, 7}, {5, 6}}), // j axis 4-fold, pi
    Permutation::CreateFromDisjunctCycles(CubeGroup::numVertices,
                                          {{0, 2, 3, 1}, {4, 6, 7, 5}}), // j axis 4-fold, 3*pi/2
    ...
    Permutation::CreateFromDisjunctCycles(CubeGroup::numVertices,
                                          {{0, 1}, {6, 7}, {2, 5}, {3, 4}}), //  2-fold,  edge 0-1
    Permutation::CreateFromDisjunctCycles(CubeGroup::numVertices,
                                          {{2, 3}, {4, 5}, {0, 7}, {1, 6}}), //  2-fold,  edge 2-3

    Permutation::CreateFromDisjunctCycles(CubeGroup::numVertices,
                                          {{0, 2}, {5, 7}, {1, 6}, {3, 4}}), //  2-fold,  edge 0-2
    Permutation::CreateFromDisjunctCycles(CubeGroup::numVertices,
                                          {{1, 3}, {4, 6}, {0, 7}, {2, 5}}), //  2-fold,  edge 1-3

    Permutation::CreateFromDisjunctCycles(CubeGroup::numVertices,
                                          {{0, 4}, {3, 7}, {1, 6}, {2, 5}}), //  2-fold,  edge 0-4
    Permutation::CreateFromDisjunctCycles(CubeGroup::numVertices,
                                          {{1, 5}, {2, 6}, {0, 7}, {3, 4}}), //  2-fold,  edge 1-5
