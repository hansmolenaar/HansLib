...
TEST(BoundingBoxTest, CreateFromListTransformed)
{
    ...
    const auto bb = BoundingBox<int, 1>::CreateFromListTransformed(
        values, [](std::array<short, 1> n) { return std::array<int, 1>{2 * n[0]}; });
    ASSERT_EQ(bb.getInterval(0).getLower(), 2);
...
{
    const auto bb =
        BoundingBox<int, 2>::CreateFromList(std::initializer_list<IntPoint2>{IntPoint2{2, 4}, IntPoint2{3, 1}});
...
{
    const auto bb1 =
        BoundingBox<int, 2>::CreateFromList(std::initializer_list<IntPoint2>{IntPoint2{2, 4}, IntPoint2{5, 0}});
    const auto bb2 =
        BoundingBox<int, 2>::CreateFromList(std::initializer_list<IntPoint2>{IntPoint2{3, 7}, IntPoint2{0, 2}});
    const auto bb3 =
        BoundingBox<int, 2>::CreateFromList(std::initializer_list<IntPoint2>{IntPoint2{10, 11}, IntPoint2{12, 13}});
    const auto bb12 = BoundingBox<int, 2>::TryGetOverlap(bb1, bb2);
    const auto expect =
        BoundingBox<int, 2>::CreateFromList(std::initializer_list<IntPoint2>{IntPoint2{2, 2}, IntPoint2{3, 4}});
...
TEST(BoundingBoxTest, Scale1)
{
    const auto bb = BoundingBox<int, 1>::CreateFromList(std::initializer_list<IntPoint1>{IntPoint1{2}, IntPoint1{1}});
    const auto scale =
        BoundingBox<int, 1>::CreateFromList(std::initializer_list<IntPoint1>{IntPoint1{5}, IntPoint1{10}});
...
{
    const auto bb = BoundingBox<Rational, 2>::CreateFromList(
        std::initializer_list<RatPoint2>{RatPoint2{Rational{0}, Rational{2}}, RatPoint2{Rational{1}, Rational{4}}});
    const auto scale = BoundingBox<Rational, 2>::CreateFrom2Points(RatPoint2{Rational{1, 5}, Rational{2, 5}},
                                                                   RatPoint2{Rational{3, 5}, Rational{4, 5}});
    const auto result = bb.scaleFrom01(scale);
...
TEST(BoundingBoxTest, Scale1Double)
{
    ...
    const auto scale =
        BoundingBox<Rational, 1>::CreateFrom2Points(RatPoint1{Rational{1, 4}}, RatPoint1{Rational{3, 4}});
    const auto result = bb.scaleFrom01(scale);
    ASSERT_EQ(result.getIntervals().size(), 1);
...
{
    const auto bb = BoundingBox<Rational, 2>::CreateFrom2Points(RatPoint2{Rational{1, 1}, Rational{1, 1}},
                                                                RatPoint2{Rational{2, 1}, Rational{5, 1}});
...
TEST(BoundingBoxTest, ScalePoint2)
{
    const auto bb = BoundingBox<Rational, 2>::CreateFromList(
        std::initializer_list<RatPoint2>{RatPoint2{Rational{0}, Rational{2}}, RatPoint2{Rational{1}, Rational{4}}});
    const auto scale = BoundingBox<Rational, 2>::CreateFrom2Points(RatPoint2{Rational{1, 5}, Rational{2, 5}},
...
TEST(BoundingBoxTest, ScalePoint1WithRational)
{
    const auto bb = BoundingBox<double, 1>::CreateFromList(std::initializer_list<Point1>{Point1{2}, Point1{1}});
    const RatPoint1 scale{Rational{5, 2}};
    const auto result = bb.scaleFromPoint01(scale);
