...
template <size_t N> static void TestIntKdTree(std::span<const Point<int, N>> points)
{
    ...
    const auto expectToFind =
        std::count_if(points.begin(), points.end(), [&points](const Point<int, N> &p) { return p == points.front(); });
...
TEST(KdTreeTest, TestPreSorting)
{
    ...
    std::sort(order.begin(), order.end(),
              [&preSorting0](KdTreePosition p0, KdTreePosition p1) { return preSorting0.less(p0, p1); });
    ...
    std::sort(order.begin(), order.end(),
              [&preSorting1](KdTreePosition p0, KdTreePosition p1) { return preSorting1.less(p0, p1); });
...
TEST(KdTreeTest, FindInRange2DFromExample)
{
    ...
    const std::array<IntPoint2, 6> points{IntPoint2{2, 3}, IntPoint2{5, 4}, IntPoint2{9, 6},
                                          IntPoint2{4, 7}, IntPoint2{8, 1}, IntPoint2{7, 2}};
    TestIntKdTree<2>(points);
