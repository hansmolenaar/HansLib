...
std::pair<Tag, std::vector<Tag>> GenerateTags(const IGraphUs &graph)
{
   ...
   {
      const TagEntry currentSize = currentItr->first.size;
      const auto itrSameSize = std::find_if_not(
          currentItr, itrPurePath, [currentSize](const auto &tagChain) { return tagChain.first.size == currentSize; });
      TagEntry count = 1;
      for (; currentItr != itrSameSize; ++currentItr, ++count)
      {
   ...
   while (currentItr != itrAttachedCycle)
   {
      ...
      const auto itrSameAttachedTo = std::find_if_not(currentItr, itrAttachedCycle, [attachedTo](const auto &tagChain) {
         return tagChain.first.attacheFirst == attachedTo;
      });
      while (currentItr != itrSameAttachedTo)
      {
         const TagEntry currentSize = currentItr->first.size;
         const auto itrSameSize = std::find_if_not(currentItr, itrSameAttachedTo, [currentSize](const auto &tagChain) {
            return tagChain.first.size == currentSize;
         });
         ...
         for (; currentItr != itrSameSize; ++currentItr, ++count)
         {
            ...
            {
               // ( ChainId, cycle size, number of cycle with this size, position in cycle )
               retval[currentItr->second.at(n)] = std::vector<TagEntry>{ChainId::AttachedCycle, currentSize, count, n};
            }
   ...
   const auto itrAttachedPathSingle = std::find_if_not(
       currentItr, chains.end(), [](const auto &tagChain) { return tagChain.first.id == ChainId::AttachedPathSingle; });
   ...
   const auto itrAttachedPathDouble = std::find_if_not(
       currentItr, chains.end(), [](const auto &tagChain) { return tagChain.first.id == ChainId::AttachedPathDouble; });
